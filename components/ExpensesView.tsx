...
const ExpensesView: React.FC<ExpensesViewProps> = ({ transactions, onTransactionsImported, budgets, onUpdateTransactionCategory, onUpdateTransactionSpendingType, onMassUpdateCategory, onMassUpdateSpendingType, categoryStructure, categoryInclusion, onPrintExpenseReport }) => {
  ...
  const handleSelectAllFromSearch = () => {
    const allIds = Object.values(groupedExpenses).flatMap(month =>
        Object.values(month.parentCategories).flatMap(parent =>
            Object.values(parent.subCategories).flatMap(sub =>
                sub.transactions.map(tx => tx.id)
            )
        )
    );
            ...
            {searchTerm.trim() && (
              <div className="bg-sky-100 border border-sky-200 text-sky-800 rounded-lg p-3 mb-6 flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="flex items-center">
                  ...
                  <span className={`text-lg font-bold ml-2 ${searchTotal < 0 ? 'text-red-700' : 'text-green-700'}`}>{formatCurrencyWhole(Math.abs(searchTotal))}</span>
                </div>
                 <div className="flex items-center gap-2">
                  <button onClick={handleSelectAllFromSearch} className="flex items-center text-xs px-3 py-1.5 bg-sky-200 hover:bg-sky-300 rounded-md shadow-sm font-medium">
                    <CheckCircleIcon className="h-4 w-4 mr-1.5" />
                    Select All Results
                  </button>
                  <button onClick={() => setSelectedTxIds(new Set())} className="flex items-center text-xs px-3 py-1.5 bg-sky-200 hover:bg-sky-300 rounded-md shadow-sm font-medium">
